...
class KVCacheSpec:
    ...
    def copy_with_new_block_size(self, block_size: int) -> Self:
        ...
        Also scales page_size_padded proportionally to keep the assertion
        in page_size_bytes valid.
        """
        extras = {}
        if (hasattr(self, 'page_size_padded')
                and self.page_size_padded is not None and self.block_size > 0):
            ratio = block_size // self.block_size
            if ratio > 1:
                extras['page_size_padded'] = self.page_size_padded * ratio
        return replace(self, block_size=block_size, **extras)
