...
def unify_kv_cache_spec_page_size(
    ...
    for layer_name, layer_spec in kv_cache_spec.items():
        if layer_spec.page_size_bytes == max_page_size:
            ...
            # Scale page_size_padded proportionally to keep the assertion
            # in page_size_bytes valid.
            extras = {}
            if (hasattr(layer_spec, 'page_size_padded')
                    and layer_spec.page_size_padded is not None):
                extras['page_size_padded'] = (
                    layer_spec.page_size_padded * ratio)
            new_spec = replace(layer_spec, block_size=new_block_size, **extras)
