...
class SamplingParams(
    ...
    def _validate_spec_decode(
        ...
        # min_p and logit_bias are not supported with speculative decoding:
        # silently reset to defaults and warn the user.
        if self.min_p > _SAMPLING_EPS:
            logger.warning(
                "min_p is not supported with speculative decoding. "
                "Resetting min_p to 0.0."
            )
            self.min_p = 0.0
        if self.logit_bias:
            logger.warning(
                "logit_bias is not supported with speculative decoding. "
                "Resetting logit_bias to None."
            )
