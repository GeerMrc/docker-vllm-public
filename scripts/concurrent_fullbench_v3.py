#!/usr/bin/env python3
"""
增强版完整并发基准+质量测试 v3 — 5 Phase 结构
  Phase 0: 上下文边界探测 (prefill 上限发现, 二分法)
...
v3 基于 v2，新增:
  --max-model-len  参数化 (不再硬编码 155648)
  --kv-concurrency  KV 并发倍率 (n=1→1.43, n=2→1.41)
  --probe-only      仅运行 Phase 0 (边界探测)
  --skip-probe      跳过 Phase 0
  自动推断 context-label (135168→132K, 155648→152K)

用法:
  python3 scripts/concurrent_fullbench_v3.py --max-model-len 135168 --kv-concurrency 1.41 --probe-only --config-label "bf16+n=2"
  python3 scripts/concurrent_fullbench_v3.py -c 1 --max-model-len 135168 --kv-concurrency 1.41 --skip-probe --config-label "bf16+n=2"
  python3 scripts/concurrent_fullbench_v3.py -c 4 --max-model-len 155648 --kv-concurrency 1.25 --config-label "bf16+n=1"
"""
...
CHARS_PER_TOKEN = 1.94  # 实测: 中文技术文本 + Qwen tokenizer
...
QUALITY_PROMPTS = [
    ...
    },
    {
        ...
        "check": lambda c: (
            ...
            ("795" in c or "calculate" in c) and
...
CONTEXT_TESTS = [
    ...
    },
    {
        ...
        "question": "太阳系有几颗行星？",
        "check": lambda c: ("八" in c or "8" in c) and ("(" in c or "（" in c),
        "desc": "中文数字+阿拉伯数字格式",
    },
]
...
def probe_context_boundary(api_key, max_model_len):
    """二分法探测 prefill 上限"""
    headers = {"Content-Type": "application/json", "Authorization": f"Bearer {api_key}"}

    # 初始粗粒度探测点
    coarse_points = [10_000, 30_000, 60_000, 90_000, 110_000, 120_000, 130_000, max_model_len]
    results = []
    max_ok = 0

    print(f"  Phase 0: 上下文边界探测 (max_model_len={max_model_len:,})")
    print(f"  {'─'*70}")
    print(f"  {'目标tok':>10} | {'结果':>6} | {'实际tok':>10} | {'TTFT(s)':>8} | {'耗时(s)':>8} | 备注")
    print(f"  {'─'*70}")

    for target in coarse_points:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                ttft = elapsed  # 非 streaming，TTFT ≈ 总耗时
                results.append({"target": target, "ok": True, "prompt_tokens": prompt_tok,
                                "ttft": round(ttft, 2), "elapsed": round(elapsed, 2), "note": ""})
                max_ok = max(max_ok, target)
                print(f"  {target:>10,} | {'OK':>6} | {prompt_tok:>10,} | {ttft:>8.2f} | {elapsed:>8.2f} |")
            else:
                elapsed = time.time() - start
                try:
                    err_body = resp.json()
                    err_msg = str(err_body.get("error", {}).get("message", ""))[:60]
                except Exception:
                    err_msg = f"HTTP {resp.status_code}"
                results.append({"target": target, "ok": False, "prompt_tokens": 0,
                                "ttft": None, "elapsed": round(elapsed, 2), "note": err_msg})
                print(f"  {target:>10,} | {'FAIL':>6} | {'—':>10} | {'—':>8} | {elapsed:>8.2f} | {err_msg}")
        except requests.exceptions.Timeout:
            elapsed = time.time() - start
            results.append({"target": target, "ok": False, "prompt_tokens": 0,
                            "ttft": None, "elapsed": round(elapsed, 2), "note": "超时 (>600s)"})
            print(f"  {target:>10,} | {'FAIL':>6} | {'—':>10} | {'—':>8} | {elapsed:>8.2f} | 超时")
        except Exception as e:
            elapsed = time.time() - start
            results.append({"target": target, "ok": False, "prompt_tokens": 0,
                            "ttft": None, "elapsed": round(elapsed, 2), "note": str(e)[:60]})
            print(f"  {target:>10,} | {'FAIL':>6} | {'—':>10} | {'—':>8} | {elapsed:>8.2f} | {str(e)[:60]}")

    # 二分细化: 找到最后成功和首次失败之间
    last_ok_target = 0
    first_fail_target = max_model_len + 1
    for r in results:
        if r["ok"]:
            last_ok_target = max(last_ok_target, r["target"])
        else:
            first_fail_target = min(first_fail_target, r["target"])

    if last_ok_target > 0 and first_fail_target <= max_model_len:
        # 在 (last_ok, first_fail) 之间二分 3 次
        lo, hi = last_ok_target, first_fail_target
        print(f"\n  二分细化: 范围 [{lo:,}, {hi:,}] tokens")
        for _ in range(3):
            mid = (lo + hi) // 2
            target_chars = int(mid * CHARS_PER_TOKEN)
            filler = build_filler(target_chars)
            content = filler + "\n请回答: 1"
            payload = {
                "model": MODEL,
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    results.append({"target": mid, "ok": True, "prompt_tokens": prompt_tok,
                                    "ttft": round(elapsed, 2), "elapsed": round(elapsed, 2), "note": "二分"})
                    max_ok = max(max_ok, mid)
                    lo = mid
                    print(f"  {mid:>10,} | {'OK':>6} | {prompt_tok:>10,} | {elapsed:>8.2f} | {elapsed:>8.2f} | 二分")
                else:
                    hi = mid
                    print(f"  {mid:>10,} | {'FAIL':>6} | {'—':>10} | {'—':>8} | {elapsed:>8.2f} | 二分")
            except Exception:
                hi = mid
                elapsed = time.time() - start
                print(f"  {mid:>10,} | {'FAIL':>6} | {'—':>10} | {'—':>8} | {elapsed:>8.2f} | 二分")

    print(f"\n  结果: prefill 上限 ≈ {max_ok:,} tokens (max_model_len={max_model_len:,})")

    # 找到 max_ok 对应的实际 prompt_tokens
    actual_max = 0
    for r in results:
        if r["ok"] and r["prompt_tokens"] > actual_max:
            actual_max = r["prompt_tokens"]

    return {"max_successful_target": max_ok, "max_actual_tokens": actual_max, "probes": results}
...
def get_context_tokens_for_concurrency(c, max_model_len, kv_concurrency):
    """根据并发级别和 KV 预算计算每个请求的目标上下文 token 数"""
    total_budget = int(max_model_len * kv_concurrency)
    per_req = min(max_model_len - 500, total_budget // c - 500)
    return max(per_req, 2000)


def bench_context(concurrency, api_key, max_model_len, kv_concurrency):
    """Phase D: 上下文质量测试"""
    headers = {"Content-Type": "application/json", "Authorization": f"Bearer {api_key}"}
    target_tokens = get_context_tokens_for_concurrency(concurrency, max_model_len, kv_concurrency)
    target_chars = int(target_tokens * 0.85 * CHARS_PER_TOKEN)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="增强版完整并发基准+质量测试 v3")
    parser.add_argument("-c", "--concurrency", type=int, required=False, choices=[1, 2, 3, 4],
                        help="并发级别 (probe-only 模式不需要)")
    parser.add_argument("--max-model-len", type=int, default=155648,
                        help="max_model_len (默认 155648=152K)")
    parser.add_argument("--kv-concurrency", type=float, default=1.25,
                        help="KV 并发倍率 (n=1→1.43, n=2→1.41, 默认 1.25)")
    parser.add_argument("--context-label", default=None,
                        help="上下文标签 (如 132K, 152K), 自动推断")
    ...
    parser.add_argument("--probe-only", action="store_true", help="仅运行 Phase 0 (边界探测)")
    parser.add_argument("--skip-probe", action="store_true", help="跳过 Phase 0")
    ...
    # 自动推断 context-label
    if args.context_label is None:
        if args.max_model_len <= 135168:
            args.context_label = "132K"
        elif args.max_model_len <= 139264:
            args.context_label = "136K"
        elif args.max_model_len <= 143360:
            args.context_label = "140K"
        elif args.max_model_len <= 148480:
            args.context_label = "145K"
        elif args.max_model_len <= 155648:
            args.context_label = "152K"
        else:
            args.context_label = f"{args.max_model_len // 1024}K"
    ...
    probe_str = " [probe-only]" if args.probe_only else ""
    c_str = f" — 并发={args.concurrency}" if args.concurrency else ""
    print(f"\n{'='*80}")
    print(f"  完整基准测试 v3 — {args.context_label} context{config_str}{c_str}{probe_str}")
    print(f"  max_model_len={args.max_model_len:,} | kv_concurrency={args.kv_concurrency}")
    print(f"{'='*80}")
    ...
    if not args.skip_probe:
        print(f"\n{'─'*80}")
        probe_result = probe_context_boundary(api_key, args.max_model_len)
        print(f"{'─'*80}")

    if args.probe_only:
        print(f"\n{'═'*80}")
        print(f"  Phase 0 完成: prefill 上限 ≈ {probe_result['max_actual_tokens']:,} tokens")
        print(f"  max_model_len={args.max_model_len:,} | {args.config_label}")
        print(f"{'═'*80}\n")
        sys.exit(0)

    if not args.concurrency:
        print("ERROR: 非 probe-only 模式需要指定 -c/--concurrency"); sys.exit(1)
    ...
    if not args.no_context_tests:
        ctx_target = get_context_tokens_for_concurrency(c, args.max_model_len, args.kv_concurrency)
        print(f"\n{'─'*80}")
        print(f"  D: 上下文质量测试 (Q9-Q13 × {c} 份, 目标 ~{ctx_target:,} tokens/req)")
        print(f"{'─'*80}")
        ctx = bench_context(c, api_key, args.max_model_len, args.kv_concurrency)
    ...
    probe_summary = ""
    if probe_result:
        probe_summary = f" | prefill上限≈{probe_result['max_actual_tokens']:,}tok"
    print(f"\n{'═'*80}")
    ctx_str = f" | 上下文质量={ctx['total_pass']}" if ctx else ""
    print(f"  汇总: {args.config_label} | ctx={args.context_label} | c={c}{probe_summary}")
